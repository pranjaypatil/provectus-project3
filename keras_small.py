...
def create_model():
    ...
    cnn_model.add(Convolution2D(32, 3, 3, activation='relu', dim_ordering='tf', border_mode='same', input_shape=(32, 32, 3), W_constraint=maxnorm(3)))
    cnn_model.add(Dropout(0.2))
    cnn_model.add(Convolution2D(32, 3, 3, activation='relu', border_mode='same', W_constraint=maxnorm(3)))
...
def main():
    model = create_model()
    model.fit(X_pd_train, label_train_ohe, nb_epoch=10,batch_size=10, verbose=2)
